(ns blasterai.malli-datomic.spec-utils
  "Based on https://github.com/dvingo/malli-code-gen/blob/main/src/main/space/matterandvoid/malli_gen/util.cljc"
  (:require
    [malli.core :as m]
    [malli.util :as mu])
...
(comment
  ...
  (-> common.spec/schema:shipping-order m/deref-all m/children (nth 2))
  (-> common.spec/schema:shipping-order m/deref-all m/children (nth 1)
      last m/deref)
  (-> common.spec/schema:shipping-order m/deref-all m/children (nth 1)
      last ref-schema?))
...
(defn enum-ref? [schema-ref]
  (= :enum (m/type (m/deref-all schema-ref))))
...
(defn coll-type-fn
  "cast malli seq type to a conforming function"
  [schema-def]
  (let [coll-type (m/type (m/deref-all schema-def))]
    (cond
      (#{:sequential} coll-type) seq
      (#{:set 'set?} coll-type) set
      (#{:vector :cat :catn :alt :altn :? :* :+ :repeat} coll-type) vec
      :else vec)))
...
(defn try-get-atomic
  ...
  [input-schema]
  (let [schema-def (-> input-schema m/deref-all)
...
(defn seq-schema->contents-atomic-predicate [seq-schema]
  ;(def s1 seq-schema)
  (let [seq-schema-def (m/deref-all seq-schema)
...
(defn composite-schema? [schema]
  (contains? composite-schema-types (m/type (m/deref-all schema))))

(defn seq-schema?
  "identifies list-like and regex sequence schemas"
  [schema]
  (contains? malli-coll-types (m/type (m/deref-all schema))))
...
(defn simple-seq-schema?
  ...
  [schema-or-ref]
  (let [schema-def (m/deref-all schema-or-ref)
...
(defn is-ref-coll?
  ...
  [prop-name et-schema]
  (let [prop-schema (m/deref-all (mu/get et-schema prop-name))
